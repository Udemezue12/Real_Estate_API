...
class RentalMessagingService:
    def __init__(self, db):
        self.db = db
        self.cache: Cache = Cache()
        self.logs: RentalViewHistoryRepo = RentalViewHistoryRepo(db)
        self.paginate: PaginatePage = PaginatePage()
        self.breaker: CircuitBreaker = CircuitBreaker()
        self.convos: RentalConversationRepo = RentalConversationRepo(db)
        self.messages: RentalEncryptedMessageRepository = (
            RentalEncryptedMessageRepository(db)
        )
        self.mapper: ORMMapper = ORMMapper()
        self.listings: RentalListingRepo = RentalListingRepo(db)
    ...
    async def schedule_viewing(
        ...
        *,
        ...
        async def handler():
            ...
            if current_user.id not in {convo.renter_id, convo.owner_id}:
            ...
            return self.mapper.one(item=convo, schema=RentalConversationOut)
    ...
    async def list_messages(
        ...
        async def handler():
            ...
            if not convo:
                raise HTTPException(404, "Conversation not found")
            cache_key = f"rental_messages:{conversation_id}:{current_user.id}"
    ...
    async def list_messages_for_cursor(
        ...
        async def handler():
            ...
            if not before:
                cache_key = (
                    f"rental_messages:latest:cursor:{conversation_id}:{current_user.id}"
                )
                cached = await self.cache.get_json(cache_key)
            if cached:
                return cached
    ...
    async def start_or_get_conversation(self, current_user, listing_id: UUID):
        async def handler():
            ...
            if not listing.is_available:
                raise HTTPException(400, "This property is currently not available")
    ...
    async def send_message(
        ...
        *,
        ...
        async def handler():
            ...
            receiver_id = (
                conversation.owner_id
                if current_user.id == conversation.renter_id
                else conversation.renter_id
            )
