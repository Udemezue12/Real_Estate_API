...
class RentalListingImageService:
    ...
    async def get_all_images(
        ...
        cache_key = f"rental_listing:{listing_id}:images"
    ...
    async def _update_image_record(
        ...
        *,
        ...
        try:
            ...
            await self._safe_delete_cloudinary(
                public_id=public_id
            )
    ...
    async def update_image(
        ...
        if old_image.created_by_id != current_user.id:
            raise HTTPException(
                status_code=403, detail="You cannot update an image you didn't create"
            )
        ...
        if existing and existing.id != image_id:
            raise HTTPException(
                status_code=400, detail="Another image with this content already exists"
            )
    ...
    async def delete_image(
        ...
        if image.created_by_id != current_user.id:
            raise HTTPException(
                status_code=403, detail="You cannot delete an image you didn't create"
            )
    ...
    async def _safe_delete_cloudinary(self, public_id: str, resource_type:str="images"):
        try:
            await self.cloudinary.delete_image(public_id=public_id, resource_type=resource_type)
        except Exception:
            pass
