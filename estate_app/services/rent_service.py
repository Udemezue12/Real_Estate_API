...
class RentService:
    ...
    def process_rent_notifications_using_celery(self):
        tenants_7 = self.tenant_repo.sync_get_tenants_expiring_in(7)
        ...
        for tenant in tenants_7:
            ...
            if not self.ledger_repo.sync_exists(tenant.id, "RENT_REMINDER_7_DAYS"):
                self.ledger_repo.sync_create(
                    tenant.id,
                    "RENT_REMINDER_7_DAYS",
                    new_value={"expires_on": str(tenant.rent_expiry_date)},
                )

            self.email_service.sync_send_rent_reminder_email(tenant.email, 7, name)
            send_sms.sync_send_rent_reminder_sms(tenant.phone, 7, name)

        tenants_3 = self.tenant_repo.sync_get_tenants_expiring_in(3)
        ...
        for tenant in tenants_3:
            ...
            if not self.ledger_repo.sync_exists(tenant.id, "RENT_REMINDER_3_DAYS"):
                self.ledger_repo.sync_create(
                    tenant.id,
                    "RENT_REMINDER_3_DAYS",
                    new_value={"expires_on": str(tenant.rent_expiry_date)},
                ...
                self.email_service.sync_send_rent_reminder_email(tenant.email, 3, latter_name)
                send_sms.sync_send_rent_reminder_sms(tenant.phone, 3, latter_name)

        expired = self.tenant_repo.sync_get_expired_active_tenants()
        ...
        for tenant in expired:
            ...
            if not self.ledger_repo.sync_exists(tenant.id, "RENT_EXPIRED"):
                ...
                self.email_service.sync_send_rent_expired_email(tenant.email, get_name)
                send_sms.sync_send_rent_expired_sms(tenant.phone, get_name)

        self.tenant_repo.sync_db_commit()
        return {"status": "Processed"}
